...
extern "C" esp_err_t agent_setup_set_refresh_token(const char *refresh_token);
                          ...
                          const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (strcmp(esp_rmaker_param_get_type(param), ESP_RMAKER_PARAM_BASE_URL) == 0) {
        ...
        // TODO: Find a better way to update refresh token to agent setup
...
esp_err_t matter_controller_update_device_list()
{
    if (!s_matter_controller_handle) {
        return ESP_ERR_INVALID_ARG;
    }
    return s_matter_controller_callback(s_matter_controller_handle, MATTER_CONTROLLER_CALLBACK_TYPE_UPDATE_DEVICE);
}
